...
function Navbar({ toggleSidebar }) {
  ...
  return (
    <nav className="navbar py-5 px-8 bg-navBgColor">
      ...
      <div className="flex-none gap-2">
        <div className="dropdown dropdown-end">
          <label tabIndex={0} className="btn btn-ghost btn-circle avatar" onClick={()=>setShowMenu(true)}>
            <div className="w-10 rounded-full">
              {isLoading?<img src={circle} />:<img src={profile?.img || profileDummy} />}
            </div>
          </label>
          {showMenu?(
            <ul
