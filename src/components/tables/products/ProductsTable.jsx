...
function ProductsTable({ data,setIsAscending }) {
  ...
  return (
    ...
    <div className="h-full flex flex-col justify-between">
        ...
        <table className="table w-full table-pin-rows table-pin-cols">
          <thead className="p-0">
            <tr className="font-bold  text-3xl text-blackHigh">
              ...
              <th className="bg-tablenav text-base text-blackMediumEmp font-poppins font-semibold py-5">Title</th>
              <th className="bg-tablenav text-base text-blackMediumEmp font-poppins font-semibold py-5">Price</th>
              <th className="bg-tablenav text-base text-blackMediumEmp font-poppins font-semibold py-5">Qty</th>
          ...
          {currentRows?.length === 0 ? (
            ...
            <tbody className="">
              {currentRows?.map((product, i) => (
                <tr className=" bg-white text-blackSemi" key={product._id}>
                  <td className="py-3">
                    {currentPage === 1 && i + 1 < 10
                      ? "0" + (rowsPerPage * (currentPage - 1) + i + 1)
                      : rowsPerPage * (currentPage - 1) + i + 1}
                  </td>
                  {product?.images&&product?.images.length>1&&<td className="py-3">
                  <img
                ...
                />
                  ...
                  <td className="py-3 text-blackLowEmp text-base font-normal font-poppins">{product?.price.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",")}$</td>
                  <td className="py-3 text-blackLowEmp text-base font-normal font-poppins">{product?.qty}</td>
                  <td className="py-3 text-blackLowEmp text-base font-normal font-poppins">
                  ...
                  <td className="py-3 text-blackLowEmp text-base font-normal font-poppins flex items-center justify-end gap-4">
                    <button type="button" onClick={() => handleNavigate(product)}>
                      <svg
                        width="24"
                        height="24"
                        viewBox="0 0 24 24"
      ...
      <div className="w-full">
        ...
        ></Pagination>
      <ConfirmationModal title={'Product'} handleStatus={handleDelete} ></ConfirmationModal>
