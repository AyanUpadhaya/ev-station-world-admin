...
function FaqTable({ data,setIsAscending }) {
  ...
  return (
    ...
    <div className="h-full flex flex-col justify-between">
        ...
        <table className="table w-full table-pin-rows table-pin-cols">
          <thead className=" p-0">
            <tr className="font-bold  text-3xl text-blackHigh">
              ...
              <th className="bg-tablenav text-base text-blackMediumEmp font-poppins font-semibold py-5">Question</th>
              <th className="bg-tablenav text-base text-blackMediumEmp font-poppins font-semibold py-5">Answer</th>
              {/* date */}
          ...
          {currentRows?.length === 0 ? (
            ...
            <tbody className="">
              {currentRows?.map((faq, i) => (
                <tr className=" bg-white text-blackSemi" key={faq._id}>
                  <td className="py-3">
                  ...
                  <td className="py-3 text-blackLowEmp text-base font-normal font-poppins">
                    {faq?.question.length>15?faq?.question.slice(0,15)+"...":faq?.question}
                  </td>
                  <td className="py-3 text-blackLowEmp text-base font-normal font-poppins">
                    {faq?.answer.length>15?faq?.answer.slice(0,15)+"...":faq?.answer}
                  </td>
                  <td className="py-3 text-blackLowEmp text-base font-normal font-poppins">
                  {new Date(faq?.timestamp*1000).toLocaleDateString({
                  year: 'numeric',
                  month: 'long',
                  day: 'numeric'
                  })}
      ...
      <ConfirmationModal title={'Faq'} handleStatus={handleDelete} ></ConfirmationModal>
