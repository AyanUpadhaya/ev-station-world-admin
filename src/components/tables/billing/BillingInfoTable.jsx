...
function BillingInfoTable({ data, setIsAscending }) {
  ...
  return (
    <div className="h-full flex flex-col justify-between">
        ...
        <table className="table w-full table-pin-rows table-pin-cols">
          <thead className=" p-0">
            <tr className="font-bold  text-3xl text-blackHigh">
              ...
              {/* date */}
              <th
                className="bg-tablenav text-base text-blackMediumEmp font-poppins font-semibold py-5 flex gap-2 cursor-pointer"
          ...
          {currentRows?.length === 0 ? (
            ...
            <tbody className="">
              {currentRows?.map((billing, i) => (
                <tr className=" bg-white text-blackSemi" key={billing._id}>
                  <td className="py-3">
                  ...
                  <td className="py-3 text-blackLowEmp text-base font-normal font-poppins">
                    {billing?.fullName?.length > 16
                      ? billing?.fullName.slice(0, 16) + "..."
                      : billing?.fullName}
                  </td>
                  <td className="py-3 text-blackLowEmp text-base font-normal font-poppins">
                    {billing?.companyName?.length > 11
                      ? billing?.companyName.slice(0, 11) + "..."
                      : billing?.companyName}
                  </td>
                  <td className="py-3 text-blackLowEmp text-base font-normal font-poppins">
                    {billing?.email}
                  ...
                  <td className="py-3 text-blackLowEmp text-base font-normal font-poppins">
                    {new Date(billing?.timestamp * 1000).toLocaleDateString({
                      year: "numeric",
                      month: "long",
                      day: "numeric",
                    })}
                  ...
                  <td className="py-3 text-blackLowEmp text-base font-normal font-poppins flex items-center justify-end gap-4">
                    ...
                    <label
                      ...
                      className="cursor-pointer"
                      onClick={() => setBillingId(billing._id)}
                    >
                      <svg
                        width="24"
                        height="24"
      ...
      <div className="w-full">
        ...
        ></Pagination>
        <BillingInfoModal billingData={selectedItem}></BillingInfoModal>
        <ConfirmationModal
          title={"Billing"}
